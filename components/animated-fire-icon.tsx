...
export interface AnimatedFireIconProps {
  size: number;
}

const largePathColorPalette = ["#ea580c", "#f97316", "#fb923c", "#fdba74", "#f59e0b", "#991b1b"];
const smallPathColorPalette = ["#d97706", "#facc15", "#fde047", "#b91c1c"];

export const AnimatedFireIcon = ({ size }: AnimatedFireIconProps) => {
  const duration = 2000;
  const largePathProgress = useSharedValue(0);
  const smallPathProgress = useSharedValue(0);

  useEffect(() => {
    largePathProgress.value = withRepeat(
      withTiming(1, {
    ...
    smallPathProgress.value = withRepeat(
      withTiming(1, {
  ...
  const largePathAnimatedProps = useAnimatedProps(() => {
    const fill = interpolateColor(
      largePathProgress.value,
      [0, 0.33, 0.66, 1],
      largePathColorPalette,
    );
    const fillOpacity = interpolate(largePathProgress.value, [0, 1], [0.6, 1.0]);
  ...
  const smallPathAnimatedProps = useAnimatedProps(() => {
    const fill = interpolateColor(
      smallPathProgress.value,
      [0, 0.33, 0.66, 1],
      smallPathColorPalette,
    );
    const fillOpacity = interpolate(smallPathProgress.value, [0, 1], [0.7, 0.9]);
  ...
  return (
    <AnimatedSvg width={width} height={height} viewBox="0 0 21 32" fill="none">
      <AnimatedPath
        animatedProps={largePathAnimatedProps}
      ...
      />
      <AnimatedPath
        animatedProps={smallPathAnimatedProps}
